...
map<string, function<size_t(size_t, size_t)>> operators = {{"ADD", ADD}, {"MUL", MUL}, {"CONCAT", CONCAT}};
...
int main()
{
  ...
  for (auto test : calibration_tests)
  {
    ...
    operations.reserve(pow(test.second.size() - 1, 3));
...
void get_permutations(int n, vector<vector<string>> &result, vector<string> permutation)
{
  ...
  vector<string> temp1 = permutation;
  vector<string> temp2 = permutation;

  permutation.push_back("ADD");
  get_permutations(n - 1, result, permutation);

  temp1.push_back("MUL");
  get_permutations(n - 1, result, temp1);

  temp2.push_back("CONCAT");
  get_permutations(n - 1, result, temp2);
}