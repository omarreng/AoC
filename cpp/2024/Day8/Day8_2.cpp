...
void process_antenna_pair(pair<int, int> a1, pair<int, int> a2, const vector<vector<char>> &input, set<pair<int, int>> &results)
{
  ...
  pair<int, int> possible_antinode_1, possible_antinode_2;

  int iteration = 0;
  do
  {
    possible_antinode_1 = make_pair(antenna_1.first - delta * iteration, line(antenna_1.first - delta * iteration));
  ...
  do
  {
    ...
    iteration++;
  } while (is_bound(input, possible_antinode_2.first, possible_antinode_2.second));
}
